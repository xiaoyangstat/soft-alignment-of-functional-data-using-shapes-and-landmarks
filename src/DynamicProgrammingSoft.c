...
double CostFn2(const double *q1L, const double *q2L,
	int kL, int lL, int iL, int n, int *lLoff, double sqrtm, double lam);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	int i, j, k, l, iL, kL, lL, n, M, N, Eidx, Fidx, Ftmp, Fmin, Num, *Path, *xy, x, y, cnt, kLoff;
	const double *q1, *q2;
	...
	for (Num = 0; Num < NNBRS; ++Num) {
		...
		for (kLoff = 0; kLoff <= MAXDJL; kLoff++){
		...
		for (i = 1; i < N; ++i) {
			...
			for (Num = 0; Num < NNBRS; ++Num) {
				...
				if (k >= 0 && l >= 0) {
					kL = SCL*k;
					lL = SCL*l;
					Etmp = E[N*l + k] + CostFn2(q1L,q2L,kL,lL,iL,n,lLoff[Num],sqrtm[Num],lam);
					if (Num == 0 || Etmp < Emin) {
						Emin = Etmp;
						Eidx = Num;
					}
...
double CostFn2(const double *q1L, const double *q2L,
               ...
               int *lLoff, double sqrtm, double lam) {
	double E = 0, tmp;
	int kLoff, idx, d;
